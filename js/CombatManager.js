  ...
  if (statsContainer) {
    ...
    statsContainer.innerHTML = `
      ...
      <div class="stats" style="display: flex; flex-direction: column; gap: 15px; font-size: 1.2rem;">
        <div class="stat hp ${isLowHP ? 'low-hp' : ''}" style="display: flex; align-items: center; gap: 10px; height: 24px;"><span class="hp-icon" style="display: flex; align-items: center; width: 24px; justify-content: center;">❤️</span> <span><span class="stat-label">Vida: </span><span>${p.hp}</span>/<span>${p.maxHp}</span> ${finalProjectedHp !== p.hp && !isCrampPhase ? `<span style="color:${finalProjectedHp < p.hp ? '#ff4d4d' : '#33cc33'}; font-size: 0.9em; margin-left: 8px;">(➔ ${finalProjectedHp}/${p.maxHp})</span>` : ''}</span></div>
        ${projectedHtml}
        ${p.shield > 0 ? `<div class="stat shield" style="display: flex; align-items: center; gap: 10px; height: 24px; color: #33cc33;" title="Escudos del Protector"><span style="display: flex; align-items: center; width: 24px; justify-content: center;">🛡️</span> <span><span class="stat-label">Escudos: </span><span>${p.shield}</span></span></div>` : ''}
        <div class="stat gold" style="display: flex; align-items: center; gap: 10px; height: 24px;"><span style="display: flex; align-items: center; width: 24px; justify-content: center;">${COIN_SVG}</span> <span><span class="stat-label">Oro: </span><span>${p.mo}</span></span></div>
        <div class="stat energy" style="display: flex; align-items: center; gap: 10px; height: 24px; color: #00d2ff;" title="Energía del Rol"><span style="display: flex; align-items: center; width: 24px; justify-content: center;">🔷</span> <span><span class="stat-label">Energía: </span><span>${p.energy}</span></span></div>
      </div>
      ${goblinSection}
    `;
  }
  ...
  c.goblins.forEach(gob => {
    let gobWrapper = document.createElement('div');
      // Forzar siempre goblin-dance-2 para corregir partidas guardadas
        ...
        gobWrapper.style.setProperty('--global-sync-delay', `-${((Date.now() % 100000) / 1000) + (gob.animationOffset || 0)}s`);
    ...
    if (gameState.activeSenda === 'horda' && gob.isBoss && !gob.isDying) {
      ...
      <div style="position: absolute; top: 4%; left: calc(33% + 35px); display: flex; align-items: center; z-index: 10;">
          ...
          <div style="background: #2a2a35; color: #eee; font-family: 'Outfit', sans-serif; font-size: 0.8rem; font-weight: bold; padding: 6px 8px 6px 10px; margin-left: -6px; border-radius: 0 4px 4px 0; border: 1px solid #555; border-left: none; box-shadow: 2px 2px 4px rgba(0,0,0,0.6); display: flex; align-items: center; z-index: 1;"><span style="font-size: 0.65rem; color: #ccc; margin-right: 4px;">EXP</span>${pexAmount}</div>
      </div>
    `;
    }
...
function renderPlayer() {
  ...
  gameState.players.forEach((p, index) => {
    ...
    if (p.isBot && p.botDNA && p.botDNA.length >= 2) {
        dnaHTML = `<div style="font-size: 0.65rem; color: #a5a5a5; background: rgba(212, 175, 55, 0.15); padding: 2px 5px; border-radius: 4px; border: 1px solid var(--gold); display: flex; align-items: center; justify-content: center;">&#129302;</div>`;
    }
  ...
  if (roleId === 'curandero') {
    ...
    if (activeGoblins.length === 0) {
      ...
      activeGoblins.forEach(gob => {
        ...
        gbtn.innerHTML = `
          <div class="goblin-hp" style="width: 44px !important; height: 44px !important; top: 14px !important; left: 32% !important; right: auto !important; font-size: 1.3rem !important; padding-bottom: 4px; box-shadow: none !important;">${gob.currentHp}</div>
