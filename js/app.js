    ...
    if (!roleData) {
      ...
      if (!isHover && setupContent && roleData.icon) {
        setupContent.style.backgroundImage = `radial-gradient(circle at top left, rgba(10, 10, 10, 0.35) 0%, rgba(10, 10, 10, 0.85) var(--bg-role-fade-1, 300px), rgba(10, 10, 10, 1) var(--bg-role-fade-2, 450px)), url('${roleData.icon}')`;
          setupContent.style.backgroundPosition = 'left top, -20px -30px';
...
function updateSetupSendaPreview() {
  ...
  if (sendaVal === 'horda') {
    html += `<div style="margin-bottom: 15px;">
      ...
      <p style="margin: 0 0 10px 0; color: #ccc; font-size: 0.9rem; line-height: 1.4;">El Señor de la Horda obtiene Puntos de Rencor (PR) al principio de cada ronda para gastarlos contra los jugadores.</p>
  ...
  if (setupSendaContent) {
    ...
    if (sendaVal === 'horda') {
      ...
      const fileNameExt = originalImage.substring(originalImage.lastIndexOf('/') + 1);
      const fileNameNoExt = fileNameExt.substring(0, fileNameExt.lastIndexOf('.'));
      imageUrl = basePath + 'i_' + fileNameNoExt + '.png';
    }
    
    if (imageUrl) {
      setupSendaContent.style.backgroundImage = `radial-gradient(circle at top left, rgba(10, 10, 10, 0.1) 0%, rgba(10, 10, 10, 0.85) var(--bg-senda-fade-1, 550px), rgba(10, 10, 10, 1) var(--bg-senda-fade-2, 800px)), url('${imageUrl}')`;
        setupSendaContent.style.backgroundPosition = 'top left, top left';
        setupSendaContent.style.backgroundSize = 'cover, var(--bg-senda-size, cover)';
...
function initSendaSelectionScreen() {
  ...
  btnHordeMode.style.cssText = 'padding: 0 !important; border-radius: 50%; width: 50px !important; height: 50px !important; min-height: 50px !important; flex-shrink: 0; background: transparent; border: 2px solid var(--accent-red); cursor: pointer; transition: all 0.3s ease; position: relative; overflow: hidden; margin: 0 15px; display: flex; align-items: center; justify-content: center; box-shadow: 0 0 10px rgba(230, 57, 70, 0.4);';
...
setInterval(() => {
  if (typeof gameState !== 'undefined' && gameState && !gameState.isResolvingWaveSequentially && !window.isAnimatingWave) {
    ...
    allGoblins.forEach(gob => {
        ...
        if (inCombat) {
            ...
            if (!gob.combatTimerSet) {
                ...
                gob.tauntNextActionTime = now + 1500 + (Math.random() * 1000); // Entre 1500 y 2500 ms
