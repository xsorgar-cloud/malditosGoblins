  ...
  gameState.battlefield.goblins.forEach(goblin => {
    ...
    if (goblin.isDying) {
      ...
      const badgeHTML = isInvulnerable 
        ...
        : '';
      let buffsHTML = '';
if (goblin.pielDeCuero > 0) buffsHTML += `<span title="Piel de Cuero (Absorbe ${goblin.pielDeCuero} de daño)" style="filter: drop-shadow(0 0 2px #fff);">🛡️${goblin.pielDeCuero > 1 ? `<b style="font-size:0.8rem; margin-left: 2px; color: #fff; text-shadow: 1px 1px 0 #000; font-family: monospace;">x${goblin.pielDeCuero}</b>` : ""}</span>`;
